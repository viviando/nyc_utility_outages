...
for (c in 1:length(characteristic)){
  ...
  for (s in 1:length(serv_type)){
    ...
    result_char_level_pct <- svyby(as.formula(paste0("~I(", serv_type[s], ")")), as.formula(paste0("~I(", characteristic[c], ")")),
                    svy_outage_ext_factors, svyciprop, method = "xlogit", na.rm = TRUE)
    
    # service outage == 1 across levels of characteristic (ci)
    # to get the ci, use vartupe = "ci" in svyby (NOT confint because this doesnt account for svyweights)
    result_char_level_ci <- svyby(as.formula(paste0("~I(", serv_type[s], ")")), as.formula(paste0("~I(", characteristic[c], ")")),
                                  svy_outage_ext_factors, svyciprop, vartype = "ci")
    
    # combine output from result into a clean data
    result_char_level <- as.data.frame(cbind(result_char_level_pct[[1]], result_char_level_pct[[2]]*100, 
                                             result_char_level_ci[[3]]*100, result_char_level_ci[[4]]*100)) %>% 
      rename(!!characteristic[c] := V1,
             pct_total = V2,
             pct_lci = V3,
             pct_uci = V4) %>%
      mutate(service_type = paste(serv_type[s])) %>% 
      mutate_at(vars(contains("pct")), as.numeric)
    
    # join all results so we can then calculate reliability metrics
    pct_serv_type_across_characteristic <- full_join(totals_per_level_in_characteristic, total_serv_type_across_characteristic) %>% #join prevalence overall characteristic estimates and characteristic-outage prevalence
      full_join(., result_char_level) %>% #join with the characteristic level percents 
    ...
    dist_char_level <- rbind(dist_char_level, pct_serv_type_across_characteristic_w_rse)
...
# save output
write_csv(dist_char_level, paste0(here(), "/output/results/dist_char_level.csv"))
